...
assetfinderfunc() {
	...
	assetfinder $url | tee output/$url/recon/assets.txt
	...
	cat output/$url/recon/subs.txt | sort -u | httprobe | tee output/$url/recon/probed.txt
	echo "[+] Probing done!"
...
dirsearchfunc() {
	...
	./dependencies/dirsearch/dirsearch.py -u $url -e php,js,json,csv,pdf,zip,backup,html,cshtml,xml,sql,nosql -w /usr/share/dirbuster/wordlists/directory-list-2.3-small.txt -t 100 --json-report=output/$url/recon/dirsearch.json
}
...
hostinfofunc() {
	...
	for line in $(cat output/$url/recon/final.txt); do nslookup $line >>output/$url/recon/host-details/host-details.txt; done
}
...
whatwebfunc() {
	...
	for line in $(cat output/$url/recon/final.txt); do ./dependencies/WhatWeb-0.5.5/whatweb -t $line >>output/$url/recon/host-details/whatweb.txt; done
}
...
gowitnessfunc() {
	...
	echo "==================================================="
	echo "----^---------^--------ⓢⓤⓑⒺⓝⓤⓜ------"
	gowitness file --source=./output/$url/recon/probed.txt --threads=6 --resolution="1200,750" --log-format=json --timeout=60 --destination="./output/$url/recon/screenshots/"
}
...
waybackfunc() {
	...
	cat output/$url/recon/final.txt | waybackurls >>output/$url/recon/wayback-data/wayback-data.txt
